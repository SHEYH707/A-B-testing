...
def validate_dataframe(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if duplicated_columns:
        raise ValueError("Датасет содержит повторяющиеся столбцы: " + ", ".join(duplicated_columns))
    ...
    if missing_columns:
        raise ValueError("Отсутствуют обязательные столбцы: " + ", ".join(missing_columns))
    ...
    if columns_missing_values:
        raise ValueError("Обнаружены пустые значения в столбцах: " + ", ".join(columns_missing_values))
    ...
    if invalid_groups:
        raise ValueError("Недопустимые значения в столбце group: " + ", ".join(invalid_groups))

    invalid_pages = sorted(set(validate_data["landing_page"]) - set(ALLOWED_PAGES))

    if invalid_pages:
        raise ValueError("Недопустимые значения в столбце landing_page: " + ", ".join(invalid_pages))

    converted_values = pd.to_numeric(validate_data["converted"], errors="coerce")

    if converted_values.isna().any():
        raise ValueError("Столбец converted должен содержать только числовые значения 0 и 1")

    if not converted_values.isin([0, 1]).all():
        invalid_values = sorted(converted_values[~converted_values.isin([0, 1])].unique())
        raise ValueError("Столбец converted содержит недопустимые значения: " + ", ".join(map(str, invalid_values)))
    ...
    if parsed_timestamp.isna().any():
        ...
        raise ValueError(f"Не удалось преобразовать timestamp в дату. Количество недопустимых значений: {invalid_count}")
