...
def figures(summary_data: pd.DataFrame, daily_data: pd.DataFrame) -> dict[str, Figure]:
    ...
    Raises:
        ValueError: Если данные для графиков отсутствуют.
    """
    if summary_data.empty:
        raise ValueError("Отсутствуют общие результаты анализа.")

    if daily_data.empty:
        raise ValueError("Отсутствуют дневные результаты анализа.")
...
def bar_chart(summary_data: pd.DataFrame) -> Figure:
    """
    ...
    bars = axes.bar(chart_data["variant"], chart_data["ctr"])
    axes.bar_label(bars, labels=[f"{value:.4f}%" for value in chart_data["ctr"]], padding=4)
    ...
    axes.set_ylim(0, maximum_ctr * 1.2 if maximum_ctr > 0 else 1)
...
def histogram(daily_data: pd.DataFrame) -> Figure:
    ...
    for group, label in group_names.items():
        group_values = daily_data.loc[daily_data["group"] == group, "ctr"]

        if group_values.empty:
            continue

        bins_count = min(10, max(1, len(group_values)))
        axes.hist(group_values, bins=bins_count, alpha=0.6, label=label)
...
def scatter_plot(daily_data: pd.DataFrame) -> Figure:
    ...
    for group, label in group_names.items():
        ...
        axes.scatter(group_data["date"], group_data["ctr"], label=label, alpha=0.8)

    unique_dates = pd.to_datetime(daily_data["date"].dropna().unique())

    if len(unique_dates) == 1:
        only_date = pd.Timestamp(unique_dates[0])
        axes.set_xlim(only_date - pd.Timedelta(days=1), only_date + pd.Timedelta(days=1))
...
def boxplot(daily_data: pd.DataFrame) -> Figure:
    """
    ...
    Raises:
        ValueError: Если отсутствует одна из групп.
    """
    control_values = daily_data.loc[daily_data["group"] == "control", "ctr"].to_numpy()
    treatment_values = daily_data.loc[daily_data["group"] == "treatment", "ctr"].to_numpy()

    if len(control_values) == 0:
        raise ValueError("Отсутствуют дневные данные варианта A.")

    if len(treatment_values) == 0:
        raise ValueError("Отсутствуют дневные данные варианта B.")

    figure, axes = plt.subplots(figsize=(8, 5), constrained_layout=True)
    axes.boxplot([control_values, treatment_values], showmeans=True)
    axes.set_xticks([1, 2], ["Вариант A", "Вариант B"])
