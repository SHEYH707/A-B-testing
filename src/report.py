...
def save_report_files(
    ...
    try:
        ...
        cleaned_data.to_csv(
            report_directory / "cleaned_data.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        daily_data.to_csv(
            report_directory / "daily_metrics.csv",
            index=False,
            encoding="utf-8-sig",
        )
        ...
        for figure_title, figure in figuree.items():
            ...
            figure.savefig(
                report_directory / file_name,
                dpi=150,
                bbox_inches="tight",
            )

    except (OSError, ValueError, VisualizationError, KeyError) as error:
        if report_directory.exists():
            shutil.rmtree(report_directory, ignore_errors=True)

        raise ReportError(f"Не удалось сформировать отчёт: {error}") from error
